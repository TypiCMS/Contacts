...
class SidebarViewComposer
{
    public function compose(View $view): void
    {
        ...
        $view->offsetGet('sidebar')->group(__(config('typicms.modules.contacts.sidebar.group', 'Contacts')), function (SidebarGroup $group): void {
            $group->id = 'contacts';
            $group->weight = 20;
            $group->addItem(__(config('typicms.modules.contacts.sidebar.label', 'Contacts')), function (SidebarItem $item): void {
                $item->id = 'contacts';
